...
def plot_backtest_results(df, trades, eq_curve, levels=None, title="Backtest Results"):
    """
    Plots the backtest results including OHLC data, buy/sell signals, and equity curve.

    Parameters:
    df (pd.DataFrame): DataFrame containing OHLC data and signals.
    trades (list): List of trades with entry and exit information.
    eq_curve (pd.DataFrame): DataFrame containing the equity curve over time.
    levels (list, optional): List of price levels to plot. Defaults to None.
    title (str, optional): Title of the plot. Defaults to "Backtest Results".
