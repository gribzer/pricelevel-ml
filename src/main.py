...
from .config import (
    PROCESSED_DATA_PATH, 
    SEQ_LEN, BATCH_SIZE, NUM_EPOCHS
)
from .data_fetcher import load_binance_data
...
def run_pipeline(device='cpu'):
    print("=== [1] Загрузка / чтение исторических данных Binance... ===")
    df = load_binance_data()
    # Убедимся, что нужные колонки есть: [close], etc.
    ...
    processed_file = os.path.join(PROCESSED_DATA_PATH, "binance_processed.csv")
    df.to_csv(processed_file)

    print("=== [4] Подготовка Dataset и DataLoader для обучения... ===")
    dataset = CryptoLevelsDataset(df, label_col='level_label')

    # Разделим на train / test (или train/val/test)
